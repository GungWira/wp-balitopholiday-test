...
class OrderTripEditFormFields extends BookingEditFormFields {
	...
	protected function map_field( $field ) {
		...
		if ( isset( $field['attributes']['data-options'] ) && is_array( $field['attributes']['data-options'] ) ) {
			...
			// For start_date field, set minDate to booked_date if available and in the future, otherwise use 'today'
			elseif ( $name === 'start_date' ) {
				if ( ! empty( $this->defaults['booked_date'] ) ) {
					$booked_date = strtotime( $this->defaults['booked_date'] );
					$today = strtotime( 'today' );
					$data_options['minDate'] = $booked_date > $today ? $this->defaults['booked_date'] : 'today';
				} else {
					$data_options['minDate'] = 'today';
				}
			}
		}
