...
/**
 * Class Booking
 * This class represents a trip booking to the Travel Primakara plugin.
 *
 * @since 6.0.0
 */
class Booking extends PostType {
	...
	public function get_labels(): array {
		return array(
			...
			'menu_name'          => _x( 'Travel Primakara', 'admin menu', 'wp-travel-engine' ),
	...
	private function handle_save( $post_id, $post, $update = false ) {
		...
		$trip_info = $request->get_param( 'order_trip' ) ?? array();
	...
	protected function get_template_args( BookingModel $booking, string $mode = 'view' ): array {
		...
		return array(
			...
			'cart_info'                      => $cart_info,
			'template_mode'                  => $mode,
			'order_trip_form_fields'         => new OrderTripEditFormFields( $order_trip_defaults, $mode ),
			...
			'payments_edit_form_fields'      => array_map(
				function ( Payment $payment ) use ( $mode, $booking, $payment_data, $payment_status ) {
	 ...
	 * @since 6.4.0
	 */
	private function process_customer( $request, int $post_id ): void {
	...
	private function process_traveller_details( BookingModel $booking, Validator $form_validator ): void {
		...
		// Filter out empty travellers.
		$filtered_data = array_filter(
			$sanitized_data,
			function ( $traveler ) {
				foreach ( $traveler as $field => $value ) {
					if ( $field !== 'pricing_category' && ! empty( $value ) ) {
						return true;
					}
				}
				return false;
		...
		$booking->set_traveller_details( array_values( $filtered_data ) );
	}
	...
	private function process_payments( BookingModel $booking, array &$cart_info ): void {
		...
		foreach ( $items as $payment_data ) {
			...
			$extra_fee = '0';
			foreach ( $fees as $fee ) {
				if ( is_numeric( $p_fee = $payment_data[ $fee ] ?? null ) ) {
