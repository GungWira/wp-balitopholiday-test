...
class Updates {
	...
	public function in_plugin_update_message( $file, $plugin_data ) {
		...
		if ( ! empty( $major_untested_plugins ) ) {
			...
			echo '</p>';
			$message = sprintf( __( "<strong>Heads up!</strong> The versions of the following plugins you're running haven't been tested with Travel Primakara %s. <br/><b>Please update them or confirm compatibility before updating Travel Primakara, or you may experience issues,</b>", 'wp-travel-engine' ), $new_version );
		...
		if ( ! empty( $minor_untested_plugins ) ) {
			...
			echo '</p>';
			$message = sprintf( __( "<strong>Heads up!</strong> The versions of the following plugins you're running haven't been tested with Travel Primakara %s.", 'wp-travel-engine' ), $new_version );
			include plugin_dir_path( WP_TRAVEL_ENGINE_FILE_PATH ) . 'admin/partials/plugin-updates/notice-untested-extensions.php';
			echo '<p style="display:none">';
		}
	}
