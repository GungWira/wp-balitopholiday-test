...
class Cart extends LegacyCart {
	...
	public function calculate_totals(): void {
		...
		if ( ! $this->booking_ref ) {
			$selected_gateway    = $this->payment_gateway ?? wptravelengine_settings()->get( 'default_gateway', 'booking_only' );
			$payment_gateway_obj = PaymentGateways::instance()->get_payment_gateway( $selected_gateway );
	 ...
	 * @since 6.7.0
	 */
	public function get_total_payable_amount(): float {
		if ( $this->is_curr_cart() ) {
			return $this->totals['payable_now'];
		}
