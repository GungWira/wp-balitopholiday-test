...
class Template_Tags extends TemplateTags {
	...
	public function get_trip_booking_summary() {
		...
		$is_triggered_manually = 'wte_resend_purchase_receipt' === ( $_REQUEST['action'] ?? '' );
		...
		foreach ( $this->order_trips as $trip ) :
			...
			<tr>
				...
				<td style="width: 50%;text-align: right;"><strong><?php echo $trip->end_datetime ? wptravelengine_format_trip_datetime( $trip->end_datetime ) : wptravelengine_format_trip_end_datetime( $trip->datetime, $trip_modal ); ?></strong></td>
			</tr>
			<tr>
	...
	protected function get_cart_v4_booking_payment_details( $booking ) {
		...
		$p_data  = $booking->get_payments_data();
		...
		$is_manual_trigger = 'wte_resend_purchase_receipt' === ( $_REQUEST['action'] ?? '' );
		...
		<tr>
			<td><strong><?php esc_html_e( 'Subtotal', 'wp-travel-engine' ); ?></strong></td>
			<td style="text-align: right;"><strong><?php echo wptravelengine_the_price( $amounts['subtotal'], false ); ?></strong></td>
		</tr>
			<?php
		...
		<?php if ( $payment_type !== 'full' && $amounts['initial_deposit'] > 0 ) { ?>
			<tr>
		...
		<tr>
			<td colspan="2">
				<span style="display: block;padding: 8px 16px;background-color: #147dfe1a;border-radius: 4px;margin: 0 -16px;">
					<strong style="width: 50%;display: inline-block;"><?php esc_html_e( 'Deposit Today', 'wp-travel-engine' ); ?></strong>
					<strong style="width: 49%;text-align: right;display: inline-block;">
					<?php
