...
class WP_Travel_Engine_Compatibility_Check {
	...
	public function update_messages( $response ) {
		if ( $this->requires_backward_processs() || ! empty( $this->updated_addons_actives() ) ) {
			...
			$html .= "<b>Heads Up!</b> Travel Primakara {$response->new_version} is a major update release and requires the following add-ons to be updated to the mentioned minimum versions to work properly.
			.<br />";
			$html .= 'Please update the addons first before updateing Travel Primakara to avoid any issues.<br /><br />';
