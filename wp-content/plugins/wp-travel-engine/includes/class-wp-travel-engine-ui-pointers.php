...
class WP_TRAVEL_ENGINE_ADMIN_UI_POINTERS {
	...
	function admin_print_footer_scripts() {
		...
		$tour = array(
			'wp_travel_engine_settings' => array(
				...
				'content'           => '<h3>' . __( 'Travel Primakara - Settings', 'wp-travel-engine' ) . '</h3>'
					. '<p><strong>' . __( 'Moved to new menu under Travel Primakara', 'wp-travel-engine' ) . '</strong></p>'
					. '<p>' . __( 'Travel Primakara Settings has been moved to new menu under menu head <strong>Travel Primakara</strong>, along with other settings such as Booking, Customers, Enquiries and other menus created by our addons. We have also updated our backend User Interface.', 'wp-travel-engine' ) . '</p>'
		...
		$tour_trip_meta    = array(
			'wp_travel_engine_setting' => array(
				'id'                => '#wpte-edit-trip',
				'content'           => '<h3>' . __( 'Travel Primakara - Trip Data', 'wp-travel-engine' ) . '</h3>'
		...
		*/
		if ( ! array_key_exists( $tab, $tour ) ) {
			...
			$id       = '#menu-posts-trip';
			$content  = '<h3>' . sprintf( __( 'Travel Primakara - Version %s', 'wp-travel-engine' ), WP_TRAVEL_ENGINE_VERSION ) . '</h3>';
			$content .= __( '<p>Welcome to New Travel Primakara!</p>', 'wp-travel-engine' );
		...
		elseif ( $tab != '' && in_array( $tab, array_keys( $tour ) ) ) {
	...
	function admin_enqueue_scripts() {
		...
		if ( false && ( $do_tour || $do_trip_pointer || true ) ) {
	 ...
	 */
	function admin_head() {
		$active_version = str_replace( '.', '', WP_TRAVEL_ENGINE_VERSION );
		?>
		<style type="text/css" media="screen">
			#pointer-primary {
				margin: 0 5px 0 0;
			}
			.wte_new_<?php echo absint( $active_version ); ?> {
				...
				setup = function () {
					...
					<?php if ( $button2 ) { ?>
						...
						jQuery ('#pointer-primary').on('click', function () {
							...
							<?php echo $function; ?> // phpcs:ignore
						});
