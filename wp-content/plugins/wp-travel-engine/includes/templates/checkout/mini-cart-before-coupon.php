	...
	<div class="wpte-bf-book-summary">
		<div class="wpte-bf-summary-wrap">
			...
			<?php foreach ( $cart_items as $key => $cart_item ) : ?>
				<div class="wpte-bf-trip-name-wrap">
					<div
						class="wpte-bf-trip-name"><?php echo esc_html( get_the_title( $cart_item['trip_id'] ) ); ?></div>
					<?php
					/**
					...
					?>
					<span
						class="wpte-bf-trip-date"><?php echo esc_html( sprintf( __( 'Starting Date: %1$s', 'wp-travel-engine' ), date_i18n( $date_format, strtotime( $cart_item['trip_date'] ) ) ) ); ?></span>
				</div>
				<table class="wpte-bf-summary-table">
					...
					foreach ( $cart_item['pax'] as $pax_label => $pax ) :
						...
						<tr>
							<td>
								<span><?php printf( esc_html__( '%1$s %2$s', 'wp-travel-engine' ), number_format_i18n( $pax ), ucfirst( $pax_label_disp ) ); ?></span>
							</td>
							<td>
								<b><?php echo wp_kses_post( wte_get_formated_price_html( $cart_item['pax_cost'][ $pax_label ], null, true ) ); ?></b>
							</td>
						</tr>
					<?php endforeach; ?>
					...
					<?php if ( isset( $cart_item['trip_extras'] ) && ! empty( $cart_item['trip_extras'] ) ) : ?>
						...
						<?php foreach ( $cart_item['trip_extras'] as $trip_extra ) : ?>
							<tr>
								<td>
									<span><?php echo esc_html( $trip_extra['qty'] ); ?> x <?php echo esc_html( $trip_extra['extra_service'] ); ?></span>
								</td>
								<td>
									<b><?php echo wp_kses_post( wte_get_formated_price_html( $trip_extra['qty'] * $trip_extra['price'], null, true ) ); ?></b>
								</td>
							</tr>
						<?php endforeach; ?>
					<?php endif; ?>
						...
						foreach ( $cart_discounts as $discount_key => $discount_item ) {
							...
							<tr class="wte-coupons-discount-calculation-tr">
								<td class="wte-coupons-discount-calculation-td">
									<span
										class="wpte-bf-total-txt"><?php esc_html_e( 'Coupon Discount :', 'wp-travel-engine' ); ?><?php echo esc_attr( $discount_item['name'] ) . ' - '; ?><?php echo isset( $discount_item ['type'] ) && 'percentage' === $discount_item ['type'] ? '(' . esc_attr( $discount_item ['value'] ) . '%)' : '(' . esc_attr( $currency ) . ' ' . esc_attr( $discount_item['value'] ) . ')'; ?></span>
								</td>
								<td class="wte-coupons-discount-calculation-td">
									...
									if ( 'fixed' === $discount_item ['type'] ) {
										...
										?>
										<span
											class="wpte-price"><?php echo esc_html( $discount_amount_actual ); ?></span>
										<?php
									} else {
					...
					<table class="wpte-bf-extra-info-table">
						...
						<tr>
							...
							<td class="wpte-dwnpay-amt">
								<b><?php echo wte_esc_price( wte_get_formated_price_html( $cart_totals['total_partial'], null, true ) ); ?></b>
							</td>
						</tr>
						<tr>
							<td><span><?php esc_html_e( 'Remaining Payment', 'wp-travel-engine' ); ?></span></td>
							<td class="wpte-remain-amt">
								<b><?php echo wte_esc_price( wte_get_formated_price_html( ( $cart_totals['sub_total'] - $cart_totals['total_partial'] ), null, true ) ); ?></b>
							</td>
						</tr>
						</tbody>
					</table>
		...
		<div class="wpte-bf-summary-total">
			...
			<div class="wpte-bf-total-price">
				...
				<span
					class="wpte-price"><?php echo wte_esc_price( wte_get_formated_price_html( $payable_now, null, true ) ); ?>
			</span>
			</div>
