...
//Condition to check if the booking id is valid and exists in the bookings array (i.e) for current logged in user.
if ( isset( $_GET['booking_id'] ) && ! empty( $_GET[ 'booking_id' ] ) ) { // phpcs:ignore
	$booking_exists = in_array( $_GET[ 'booking_id' ], $bookings );
	if ( $booking_exists ) {
		$booking = sanitize_text_field( intval( $_GET[ 'booking_id' ] ) );
	} else {
		//If the booking id is not valid, redirect to the dashboard page.
		wp_redirect( get_permalink( $wp_travel_engine_dashboard_id ) );
		exit;
	}
}
