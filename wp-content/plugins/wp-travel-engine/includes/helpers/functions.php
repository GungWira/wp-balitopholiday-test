...
/**
 * Checks if a Travel Primakara add-on is active.
 *
...
/**
 * Get Travel Primakara Page URLs.
 *
...
/**
 * Get Travel Primakara Email Instance.
 *
 * @return Email\Email
 *
 * @since 6.5.0
...
function wte_get_engine_extensions() {
	...
	foreach ( $plugins as $file => $plugin ) {
		if ( 'WordPress Travel Booking Plugin - Travel Primakara' !== $plugin['Name'] && ( stristr( $plugin['Name'], 'Travel Primakara' ) || stristr( $plugin['Description'], 'Travel Primakara' ) ) ) {
			$matches[ $file ] = $plugin;
		}
	}
...
function wptravelengine_system_info() {
	...
	if ( is_array( $plugins ) ) {
		foreach ( $plugins as $plugin ) {
			if ( preg_match( '/(Travel Primakara)/', $plugin['Name'] ) ) {
