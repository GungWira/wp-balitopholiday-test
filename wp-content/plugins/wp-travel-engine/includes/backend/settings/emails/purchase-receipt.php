		...
		<?php if ( version_compare( get_option( 'payment_notification_customer_version', '1.0.0' ), '2.0.0', '<' ) ) : ?>
			<div style="margin-left: 145px;" class="wpte-info-block _wte_update_notice_430">
				...
				<p>
					<?php
					echo wp_kses(
						sprintf( __( 'This is the default template from previous Travel Primakara versions. The template has been updated since %1$sv4.3.0%2$s. %3$sClick here%4$s to update', 'wp-travel-engine' ), '<code>', '</code>', '<a href="#" data-target="wte-payment-notification-customer" class="wte-email-template-updater">', '</a>' ),
						array(
							'code' => array(),
							'a'    => array(
								'href'        => array(),
