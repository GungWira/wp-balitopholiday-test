<?php
/**
 * Travel Primakara Archive Hooks
 *
...
class Wp_Travel_Engine_Archive_Hooks {
		...
		if ( ! is_admin() && $query->is_main_query() ) {
			...
			if ( $query->is_post_type_archive() && $this->show_featured_trips_on_top() ) {
				...
				if ( is_array( $featured_trips ) ) {
					$post_not_in = array_unique( array_merge( $post_not_in, $featured_trips ) );
				}
			}
	...
	/**
	 * Featured Trips sticky section for Travel Primakara Archives.
	 *
	 * @since 1.0.0
	...
	/**
	 * Header filter section for Travel Primakara Archives.
	 *
	 * @since 1.0.0
	 * @return void
	 */
