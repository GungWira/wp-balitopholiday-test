...
<div class="wrap wte_welcome__container">
	<?php if ( $header ) : ?>
		<div class="wte_about__header">
			<div class="wte_about__header-title">
				<h1>
					<span><?php echo esc_html( $data->header->title ); ?></span>
				</h1>
				<span></span>
			</div>
	...
	if ( $sections ) :
		foreach ( $sections as $section ) :
			...
			?>
			<div <?php echo esc_attr( implode( ' ', $attributes ) ); ?>>
				<div class="column">
					<?php isset( $section->title ) && printf( '<h2>%s</h2>', esc_html( $section->title ) ); ?>
					<?php isset( $section->content ) && printf( '<p>%s</p>', esc_html( implode( '</p><p>', $section->content ) ) ); ?>
					<?php
					if ( isset( $section->images ) ) {
						foreach ( $section->images as $image ) {
							printf(
								...
								esc_url( str_replace( '%assets_path_url%', "{$assets_path_url}", $image->src ) ),
								isset( $image->caption ) ? '<figcaption>' . esc_html( $image->caption ) . '</figcaption>' : ''
							);
						}
					}
					?>
	...
	<div class="return-to-dashboard">
		...
		?>
		"><?php esc_html_e( 'Go to Dashboard → Home', 'wp-travel-engine' ); ?></a>
	</div>
</div>
