<?php
/**
 * Travel Primakara - Export Booking Data
 *
 * @package WP_Travel_Engine
 */

/**
 * Travel Primakara Booking Export
 *
 * @since 5.7.4
 */
class WP_Travel_Engine_Booking_Export {
	...
	private static function safe_unserialize( $data ) {
		...
		// Try JSON first (preferred method for future compatibility).
		$json_data = json_decode( $data, true );
		if ( json_last_error() === JSON_ERROR_NONE ) {
			return $json_data;
		}

		// Fall back to unserialize with restricted classes for backward compatibility.
	...
	public function export_query( $start_date, $end_date, $booking_status, $filter_ids ) {
		...
		if ( ! empty( $filter_ids ) ) {
			...
			$sql .= $wpdb->prepare( " AND p.ID IN ($placeholders)", $filter_ids );
		}
	...
	public function data_export( $queries_data ) {
		...
		foreach ( $queries_data as $data ) {
			...
			if ( isset( $data->billinginfo ) ) {
				$unserializedBillingData = self::safe_unserialize( $data->billinginfo );
				
				if ( is_array( $unserializedBillingData ) ) {
