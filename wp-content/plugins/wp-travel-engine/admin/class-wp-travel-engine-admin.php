...
class Wp_Travel_Engine_Admin {
	...
	public function admin_menu() {
		...
		$menus = array(
			...
			'class-wp-travel-engine-admin.php'   => array(
				'parent_slug' => 'edit.php?post_type=booking',
				'page_title'  => __( 'Travel Primakara Admin Settings', 'wp-travel-engine' ),
			...
			'extensions'                         => array(
				'parent_slug' => 'edit.php?post_type=booking',
				'page_title'  => __( 'Extensions for Travel Primakara.', 'wp-travel-engine' ),
			...
			'themes'                             => array(
				'parent_slug' => 'edit.php?post_type=booking',
				'page_title'  => __( 'Themes for Travel Primakara.', 'wp-travel-engine' ),
	...
	function wpte_admin_bar( \WP_Admin_Bar $admin_bar ) {
		...
		$admin_bar->add_menu(
			array(
				...
				'parent' => null,
				'group'  => null,
				'title'  => "<span class=\"wpte-admin-menu\">{$wpte_menu_svg}</span> Travel Primakara",
				'href'   => admin_url( 'edit.php?post_type=booking&page=wptravelengine-admin-page' ),
				'meta'   => array(
					'title' => __( 'Travel Primakara', 'wp-travel-engine' ),
				),
			)
		);
		$admin_bar->add_menu(
		...
		$admin_bar->add_menu(
			array(
				...
				'meta'   => array(
					'title' => __( 'Travel Primakara Settings', 'wp-travel-engine' ),
				),
			)
		);
		$admin_bar->add_menu(
			array(
				...
				'meta'   => array(
					'title' => __( 'Travel Primakara Extensions', 'wp-travel-engine' ),
				),
			)
		);
		$admin_bar->add_menu(
			array(
				...
				'meta'   => array(
					'title'  => __( 'Travel Primakara Tutorials', 'wp-travel-engine' ),
					'target' => '__blank',
				),
			)
		);
		$admin_bar->add_menu(
			array(
				...
				'meta'   => array(
					'title'  => __( 'Travel Primakara Support', 'wp-travel-engine' ),
					'target' => '__blank',
				),
			)
		);
	...
	function wp_travel_engine_booking_custom_columns( $column, $post_id ) {
		...
		if ( 'booking' === $booking->post_type ) {
			...
			if ( $_booking_modal->is_curr_cart() ) {
				$_totals       = $_booking_modal->get_payments_data()['totals'] ?? array();
				$_paid_amount  = $_totals['total_paid'] ?? 0;
				$_due_amount   = $_totals['due_exclusive'] ?? 0;
			} else {
		...
		$column_actions = array(
			...
			'trip_date'      => function ( $booking ) use ( $trip_date ) {
				if ( $trip_date ) {
					$trip_date = date_i18n( get_option( 'date_format' ), strtotime( $trip_date ) );
				} else {
					$trip_date = wptravelengine_trip_date( $booking->ID );
				}
				...
				case 'spent':
					...
					if ( is_array( $wp_travel_engine_booked_settings ) ) {
						foreach ( $wp_travel_engine_booked_settings as $value ) {
							$booking = wptravelengine_get_booking( $value );
							if ( $booking ) {
								$value   = (string) $booking->get_total_paid_amount();
	...
	*/
	...
	public function messages_page() {
		...
		add_submenu_page(
			...
			'Travel Primakara Admin Messages',
			$menu_title,
			'manage_options',
			'wte-messages',
			array(
	...
	/**
	 * Registers settings for Travel Primakara.
	 *
	 * @since 1.0.0
	 */
	 ...
	 * @since 1.0.0
	 */
	public function wp_travel_engine_callback_function() {
		require_once plugin_dir_path( WP_TRAVEL_ENGINE_FILE_PATH ) . 'includes/class-wp-travel-engine-settings.php';
		$admin_settings_builder = new AdminSettings();
		wp_add_inline_script(
			'wptravelengine-settings',
			'var wptravelengineSettingAppConfig = ' . wp_json_encode( $admin_settings_builder->tabs() ) . ';',
			'before'
		);
	...
	function wpte_filter_admin_page_templates( $templates ) {
		...
		$templates[ 'template-checkout.php' ]               = esc_html__( 'Travel Primakara - Checkout', 'wp-travel-engine' );
	...
	*/
	...
	function wp_travel_engine_rating_notice() {
		...
		if ( get_user_meta( $user_id, 'wp-travel-engine-rating-notice', true ) != 'true' ) {
			$link_plugin = '<a href="https://wordpress.org/plugins/wp-travel-engine/" target="_blank">Travel Primakara</a>';
	...
	public function wp_travel_engine_dashboard_menu() {
		// add_menu_page( __( 'Travel Primakara', 'wte' ), __( 'Travel Primakara', 'wte' ), 'manage_options', 'wp-travel-engine-dashboard', array( $this, 'wp_travel_engine_dashboard' ), null, 40 );
		global $submenu;
		unset( $submenu[ 'edit.php?post_type=booking' ][ 10 ] ); // Removes 'Add New'.
	}
	...
	private function display_opt_in_notice_for_message_feature() {
		...
		<div class="wte-admin-notice notice notice-info is-dismissible" style="padding-bottom: 10px;">
			<p>
				<strong><?php esc_html_e( 'Travel Primakara Message: ', 'wp-travel-engine' ); ?></strong><?php esc_html_e( 'Get messages about new update releases, upcoming features, and exciting offers from Travel Primakara?', 'wp-travel-engine' ); ?>
			</p>
			<p>
				<i><?php esc_html_e( 'Note: By clicking yes, you will get an additional messages menu inside Trips menu that shows release notes, update notifications and new offers with helpful links. This will also let the plugin anonymously collect usage information to help Travel Primakara team improve the product.', 'wp-travel-engine' ); ?></i>
			</p>
					...
					<?php
						...
						<div class="wpte-alert">
							...
							// Translators: %1$s: Addon download link.
								sprintf(
									__( 'There are no <b>Travel Primakara Addons</b> installed on your site currently. To extend features and get additional functionality settings,  <a target="_blank" href="%1$s">Get Addons Here</a>', 'wp-travel-engine' ),
			...
			<div class="wpte-info-block">
				...
				<p>
					...
					if ( ! class_exists( 'Wte_Itinerary_Downloader' ) ) {
						...
						echo wp_kses( __( 'You can configure Itinerary Downloader via <b>Travel Primakara > Settings > Extensions > Itinerary Downloader</b>.', 'wp-travel-engine' ), array( 'b' => array() ) );
					}
					?>
				</p>
			</div>
	...
	public function wte_add_dashboard_widget() {
		...
		if ( in_array( 'administrator', $current_user->roles ?? [] ) ) {
			wp_add_dashboard_widget(
				...
				'Travel Primakara Booking Summary',
				array(
					$this,
