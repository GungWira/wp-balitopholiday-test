 ...
 * @author     Travel Primakara <https://wptravelengine.com/>
 */
class Wp_Travel_Engine_Public {
	...
	public static function wp_add_trip_cart() {
		...
		} else if ( ! array_key_exists( $_SESSION[ 'cart_item' ][ wte_clean( wp_unslash( $_REQUEST[ 'trip_id' ] ) ) ], $_SESSION[ 'cart_item' ] ) ) {
			...
			<div class="payment-method">
				<div class="payment-options">
					...
					<div class="payment-options-holder">
						...
						<img
							src="<?php echo esc_url( WP_TRAVEL_ENGINE_URL . '/public/css/icons/americanexpress.png' ); ?>">
			...
			foreach ( $matches as $key => $value ) {
				...
				<div class="wp-travel-engine-order-form-wrapper"
					...
					<div class="wp-travel-engine-order-right-column">
						<h3 class="trip-title"><?php echo esc_html( get_the_title( $value ) ); ?><input type="hidden"
																										name="trips[]"
																										value="<?php echo esc_attr( $value ); ?>">
						</h3>
						<ul class="trip-property">
							<li><span><?php esc_html_e( 'Start Date: ', 'wp-travel-engine' ); ?></span> <input
									type="text" min="1" class="wp-travel-engine-price-datetime"
									id="wp-travel-engine-trip-datetime-<?php echo esc_attr( $value ); ?>"
									name="trip-date[]"
									placeholder="<?php esc_html_e( 'Pick a date', 'wp-travel-engine' ); ?>"></li>
							<li class="trip-price-holder">
