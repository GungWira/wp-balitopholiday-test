...
if (function_exists('DEMO_IMPORTER_PLUS_setup')) {
    ...
    'demo_importer_plus_localize_vars',
        function($data){
            $data['importFailedMessage'] = '<p>Your website is facing a temporary issue in connecting the template server. Please try again later.</p>';
            $data['importFailedRequiredPluginsMessage'] = '<p>Your website is facing a temporary issue in connecting the template server. Please try again later.</p>';
            return $data;
        }
    );
}